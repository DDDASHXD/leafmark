...
import { mergedYamlDocument, normalizeConfig, parseFrontmatterYaml, resolveBibliographyPaths, resolveCoverPdfPath, resolveLatexTemplatePath, writeAuthorLatexFile, writeBuildLatexIncludes, } from './thesis-meta.js';
...
function looksLikeTargetFolder(value) {
    if (CHAPTER_FILE_RE.test(basename(value)))
        return false;
    if (value === '.' || value === '..')
        return true;
...
function discoverWorkspace(targetArg) {
    ...
    die(`Missing _frontmatter.md. Expected ${directFrontmatter} or ${join(legacyProject, '_frontmatter.md')}.`, 1);
}
...
async function ensureFirstRunTools(opts) {
    if (opts.skipToolsCheck || opts.command === 'init')
        return;
...
function writePdfFontSnippet(distDir) {
...
function splitBundleAndChapters(positional, workspace) {
    if (positional.length === 0)
        return { bundleName: null, chapterArgs: [] };
    const first = positional[0].trim();
    if (CHAPTER_FILE_RE.test(first))
    ...
    die(`Unknown first argument "${first}". Expected a bundle folder with _frontmatter.md or an N-*.md chapter file.`, 1);
}
...
function resolveChapterFiles(requested, projectDir) {
    if (requested.length === 0) {
        const all = listChapters(projectDir);
        if (all.length === 0)
            die(`No chapter files matching N-*.md in ${projectDir}`, 1);
        return all;
    }
    const seen = new Set();
    const out = [];
    for (const raw of requested) {
        ...
        if (!CHAPTER_FILE_RE.test(name))
            die(`Not a chapter file (expected N-name.md): ${name}`, 1);
...
function countMergedBody(merged) {
...
function buildMergedMarkdown(meta, rawYaml, chapterFiles, projectDir, mergedYamlOptions) {
...
async function withWaveLine(label, work) {
    if (!shouldShowWave()) {
...
function spawnComplete(cmd, args, options) {
...
async function runPandocPdf(merged, meta, bibPaths, extraMeta, outputPdfAbs, ctx, mergedFile, latexTemplate, useThesisHeaderIncludes, useDefaultGeometry) {
...
async function runPandocHtml(merged, meta, bibPaths, ctx, mergedFile, htmlOutAbs) {
...
async function buildOnce(workspace, opts) {
    if (!which('pandoc'))
    ...
    if (!opts.htmlOnly) {
        await runPandocPdf(merged, meta, bibPaths, extraMeta, pandocPdfOut, ctx, mergedFile, latexTemplate, useThesisHeaderIncludes, useDefaultGeometry);
...
async function watch(workspace, opts) {
    let running = false;
    let pending = false;
    const run = async () => {
        if (running) {
