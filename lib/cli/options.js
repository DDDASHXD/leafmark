...
import { DEFAULT_OUTPUT_FORMAT, parseOutputFormat } from '../build/output-formats.js';
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (a === '--output') {
            ...
            if (!value || value.startsWith('-'))
                die('--output requires a directory path', 1);
            outputDir = value;
            continue;
        }
        if (a === '--output-format') {
            const value = args[++i];
            if (!value || value.startsWith('-'))
                die('--output-format requires a value (e.g. pdf, docx)', 1);
        ...
        if (a === '--config-file') {
            const value = args[++i];
            if (!value || value.startsWith('-'))
                die('--config-file requires a JSON file path', 1);
