...
export function normalizeOrcidId(raw) {
    const t = raw.trim();
    if (!t)
        return null;
    ...
    if (ORCID_ID_RE.test(t))
        return t.toUpperCase();
    return null;
}
function parseAuthorLine(x) {
    if (typeof x === 'string') {
        const t = x.trim();
        if (!t)
            return null;
...
export function authorEntryHasOrcid(entry) {
    return entry.some((line) => typeof line !== 'string');
}
export function authorEntriesHaveOrcid(entries) {
    return entries.some(authorEntryHasOrcid);
}
function firstAuthorNameLine(entry) {
 ...
 */
export function parseAuthorEntries(v) {
    if (v === undefined || v === null)
        return [];
    if (typeof v === 'string') {
        const t = v.trim();
...
export function markdownLineToLatex(line) {
...
function splitAuthorEntryLines(lines) {
    const textLines = [];
    let orcidId = null;
...
function formatAuthorLatexBlock(lines) {
    ...
    if (orcidId)
        first = `${first}\\hspace{0.35em}\\orcidlink{${orcidId}}`;
    parts.push(first);
    for (let i = 1; i < latexText.length; i++)
        parts.push(latexText[i]);
    return parts.join('\\\\[0.4em]\n');
}
/** LaTeX `\\author{…}` with `\\and` between authors and stacked lines per author. */
export function buildAuthorLatexCommand(entries) {
...
export function writeAuthorLatexFile(meta, distDir) {
    if (!meta.authorEntries.length)
        return null;
...
function headCell(kind, raw) {
    if (!raw)
        return `\\fancyhead[${kind}]{}`;
    if (kind === 'C') {
...
function footCell(kind, inner) {
    return `\\fancyfoot[${kind}]{${inner}}`;
}
function footerLeftLine(meta) {
...
function innerFancy(meta, indent) {
    return [
        `${indent}\\fancyhf{}`,
...
function fancyHeadersBlock(meta) {
    return [
        '\\pagestyle{fancy}',
        innerFancy(meta, ''),
...
export function parseFrontmatterYaml(filePath) {
...
export function normalizeConfig(raw) {
    ...
    if (str(raw.title))
        o.title = str(raw.title);
    if (str(raw.subtitle))
        o.subtitle = str(raw.subtitle);
    ...
    if (strArray(raw.keywords).length)
        o.keywords = strArray(raw.keywords);
    if (raw.toc !== undefined)
        o.toc = bool(raw.toc, DEFAULT_META.toc);
    ...
    if (str(raw['toc-title']))
        o.tocTitle = str(raw['toc-title']);
    if (str(raw['references-title']))
        o.referencesTitle = str(raw['references-title']);
    if (raw['number-sections'] !== undefined) {
        o.numberSections = bool(raw['number-sections'], DEFAULT_META.numberSections);
    }
    if (str(raw['header-left']))
        o.headerLeft = str(raw['header-left']);
    if (str(raw['header-center']))
        o.headerCenter = str(raw['header-center']);
    if (str(raw['header-right']))
        o.headerRight = str(raw['header-right']);
    if ('footer-left' in raw)
        o.footerLeft = strOpt(raw['footer-left']);
    if (str(raw['footer-center']))
        o.footerCenter = str(raw['footer-center']);
    if ('footer-right' in raw)
        o.footerRight = strOpt(raw['footer-right']);
    if (raw['title-page'] !== undefined) {
        o.titlePage = bool(raw['title-page'], DEFAULT_META.titlePage);
    }
    if (raw.hyphens !== undefined)
        o.hyphens = bool(raw.hyphens, DEFAULT_META.hyphens);
    return o;
}
/** Resolve bibliography paths; empty array means no citeproc. */
export function resolveBibliographyPaths(raw, projectRoot) {
 ...
 */
export function resolveCoverPdfPath(raw, projectRoot) {
...
export function resolveLatexTemplatePath(raw, projectRoot) {
...
export function mergedYamlDocument(meta, raw, options) {
    const doc = {
        title: meta.title,
        ...
        hyphens: meta.hyphens,
    };
    if (meta.keywords.length)
        doc.keywords = meta.keywords;
    if (meta.authorEntries.length)
        doc['author-entries'] = meta.authorEntries;
    ...
    if (raw.lang)
        doc.lang = raw.lang;
    if (raw.abstract)
        doc.abstract = raw.abstract;
    for (const [k, v] of Object.entries(raw)) {
        if (MERGED_YAML_BLOCKED.has(k))
            continue;
        if (MERGED_YAML_FROM_META.has(k))
            continue;
        if (k in doc)
            continue;
        doc[k] = v;
...
export function writeBuildLatexIncludes(meta, distDir, srcDir, fontsRelToRoot, options = {}) {
...
//# sourceMappingURL=thesis-meta.js.map