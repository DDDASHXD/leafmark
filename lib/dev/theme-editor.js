...
function cleanup() { server.close(); rmSync(scratch, { recursive: true, force: true }); }
...
async function render(request, response) {
    try {
        ...
        if (result.status !== 0)
            return json(response, 422, { error: cleanError(result.stderr || result.stdout) });
...
function body(request) {
    return new Promise((resolveBody, reject) => { let value = ''; request.setEncoding('utf-8'); request.on('data', chunk => { value += chunk; if (value.length > 200_000)
        reject(new Error('Request is too large.')); }); request.on('end', () => resolveBody(value)); request.on('error', reject); });
}
function html(response) { response.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' }); response.end(UI); }
function json(response, status, value) { if (response.headersSent)
    return; response.writeHead(status, { 'content-type': 'application/json' }); response.end(JSON.stringify(value)); }
function cleanError(value) { return value.replace(/\x1b\[[0-9;]*m/g, '').trim().slice(-4000); }
function sampleMarkdown() {
    return `# A practical theme specimen
