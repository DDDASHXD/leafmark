...
const uiRoot = join(packageRoot, "src", "dev", "theme-editor-ui", "vite-app", "dist");
...
const server = createServer((request, response) => {
    if (request.method === "GET")
        return staticFile(request.url ?? "/", response);
...
async function render(request, response) {
    try {
        ...
        writeFileSync(join(project, ".leafmark", "config.json"), JSON.stringify({
            order: ["sample.md"],
            frontmatter: false,
            metadata: {
        ...
        const result = await spawnComplete(process.execPath, [
            tsx,
            cli,
            project,
            "sample.md",
...
function staticFile(url, response) {
    const requested = url === "/"
        ? "index.html"
