...
import type { LeafmarkConfig, LeafmarkFontFiles, LeafmarkPluginConfig } from '../workspace/config.js';
...
export function texPath(p: string): string {
  return p.replace(/\\/g, '/');
}
...
export function fontsTexRelFromDist(ctx: BuildContext): string {
  return relative(ctx.rootForRelativePaths, join(ctx.distDir, '_pandoc-fonts.tex')).replace(/\\/g, '/');
}

export function pandocResourcePath(ctx: BuildContext): string {
...
export function writePdfFontSnippet(ctx: BuildContext): void {
...
function latexIncludeLines(ctx: BuildContext, customInclude: string): string[] {
  const includePath = resolveConfigPath(ctx.activeProjectDir, customInclude);
  if (!includePath || !existsSync(includePath)) die(`Font LaTeX include not found: ${customInclude}`, 1);
  return [`\\input{${texPath(includePath)}}`];
}

function pdfFontFileLines(ctx: BuildContext): string[] | null {
  const main = ctx.config.fonts?.pdfFiles;
  const mono = ctx.config.fonts?.monoFiles;
  if (!main && !mono) return null;
  return [
...
function fontFileCommand(ctx: BuildContext, command: 'setmainfont' | 'setmonofont', font: LeafmarkFontFiles): string[] {
...
function resolveFontFile(ctx: BuildContext, font: LeafmarkFontFiles, file: string): string {
  const value = font.path ? join(font.path, file) : file;
  const p = resolveConfigPath(ctx.activeProjectDir, value);
  if (!p || !existsSync(p)) die(`Font file not found: ${value}`, 1);
  return p;
}

function defaultPdfFontLines(): string[] {
...
export async function runPandocPdf(params: {
  ...
  if (params.useDefaultGeometry) pandocArgs.push('-V', 'geometry=left=3cm,right=3cm,top=2cm,bottom=2cm');
...
export async function runPandocHtml(params: {
  ...
  if (generatedThemeCss) pandocArgs.push('--css=' + texPath(generatedThemeCss));
...
function addBibliographyArgs(pandocArgs: string[], bibPaths: string[]): void {
...
function onePluginArgs(ctx: BuildContext, plugin: LeafmarkPluginConfig, format: PandocPluginFormat): string[] {
  ...
  if (filter) {
    ...
    if (p) out.unshift('--lua-filter=' + texPath(p));
  }
  return out;
}

export function defaultLatexTemplate(ctx: BuildContext): string {
