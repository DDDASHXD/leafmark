...
export function initThemeFolder(targetArg: string | undefined): void {
  ...
  if (existsSync(baseThemeDir)) cpSync(baseThemeDir, leafmarkDir, { recursive: true });
  else {
...
function removeStarterProjectConfig(root: string): void {
  for (const path of [configPath(root), legacyConfigPath(root)]) {
    ...
    if (isStarterProjectConfig(parsed)) rmSync(path, { force: true });
  }
}

function isStarterProjectConfig(value: unknown): boolean {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const config = value as LeafmarkConfig;
...
export function useTheme(projectDir: string, themeArg: string | undefined): void {
  if (!themeArg || !themeArg.trim()) die('Usage: leafmark theme use <theme name | GitHub URL>', 1);
...
function builtinThemes(): ThemeSource[] {
  if (!existsSync(BUILTIN_THEMES_DIR)) return [];
  return readdirSync(BUILTIN_THEMES_DIR)
    .map((name) => builtinTheme(name))
    .filter((theme): theme is ThemeSource => Boolean(theme))
    .sort((a, b) => a.name.localeCompare(b.name));
}

function builtinTheme(name: string): ThemeSource | null {
  const root = join(BUILTIN_THEMES_DIR, name);
  const leafmarkDir = join(root, LEAFMARK_DIR);
  if (!existsSync(leafmarkDir) || !statSync(leafmarkDir).isDirectory()) return null;
  return { name, leafmarkDir, manifest: readThemeManifest(leafmarkDir, name) };
}

function githubTheme(url: string): ThemeSource {
...
function readThemeManifest(leafmarkDir: string, fallbackName: string): ThemeManifest {
  const p = join(leafmarkDir, THEME_CONFIG_FILE);
  if (!existsSync(p)) return { name: fallbackName };
  const parsed = JSON.parse(readFileSync(p, 'utf-8')) as unknown;
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error(`Invalid theme manifest: ${p}`);
  }
  return { name: fallbackName, ...(parsed as ThemeManifest) };
}

function copyThemeFiles(sourceLeafmarkDir: string, targetThemeDir: string): void {
  for (const entry of readdirSync(sourceLeafmarkDir)) {
    if (entry === THEME_CONFIG_FILE || entry === 'config.json') continue;
    cpSync(join(sourceLeafmarkDir, entry), join(targetThemeDir, entry), { recursive: true });
  }
}

function mergeThemeConfig(current: LeafmarkConfig, theme: LeafmarkConfig): LeafmarkConfig {
...
function ensureBaseThemeFile(target: string, section: string, fileName: string, fallback: string): void {
  if (existsSync(target)) return;
  const source = join(BUILTIN_THEMES_DIR, 'default', LEAFMARK_DIR, section, fileName);
  if (existsSync(source)) cpSync(source, target);
  else writeFileSync(target, fallback, 'utf-8');
}
...
function themeInitManifest(): ThemeManifest {
  return {
    name: 'my-theme',
...
function themeInitProjectConfig(): LeafmarkConfig {
  return {
...
function themeInitLatexInclude(): string {
  return [
...
function themeInitFrontmatter(): string {
  return [
    '---',
...
function themeInitIntroduction(): string {
  return [
    '# Introduction',
    '',
...
function themeInitMethod(): string {
  return [
    '# Method',
    '',
...
function themeInitDesignSamples(): string {
  return [
    '# Design Samples',
    '',
...
function themeInitInstructions(): string {
  return [
...
function fallbackPandocTemplate(): string {
  return [
