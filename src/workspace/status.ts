...
function metadataPlainText(raw: Record<string, unknown>): string {
  ...
  if (meta.title) parts.push(stripMarkdownToPlainText(meta.title));
  if (meta.subtitle) parts.push(stripMarkdownToPlainText(meta.subtitle));
  ...
  for (const entry of meta.authorEntries) {
    for (const line of entry) {
      if (typeof line === 'string') parts.push(stripMarkdownToPlainText(line));
      else parts.push(line.orcid);
    }
  }
...
function chapterPlainText(chapterFiles: string[], projectDir: string): string {
  const parts: string[] = [];
