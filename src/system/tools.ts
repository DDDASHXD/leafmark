...
export function kpsewhich(fname: string): string | null {
  try {
...
export function pandocHighlightArg(): string {
  try {
...
export async function ensureFirstRunTools(opts: CliOptions): Promise<void> {
  if (opts.skipToolsCheck || opts.command === 'init' || opts.command === 'order') return;
...
export function requiredToolStatus(): Array<{ name: string; command: string; available: boolean }> {
  return [
...
function installPlan(names: string[]): { command: string; args: string[] } | null {
  ...
  if (os === 'linux') {
    if (which('apt-get')) return linuxPlan('apt-get', ['install', '-y'], names);
    if (which('dnf')) return linuxPlan('dnf', ['install', '-y'], names);
    if (which('pacman')) {
      const packages = [];
      if (names.includes('pandoc')) packages.push('pandoc');
...
function linuxPlan(manager: 'apt-get' | 'dnf', installArgs: string[], names: string[]) {
  const packages = [];
  if (names.includes('pandoc')) packages.push('pandoc');
  if (names.includes('latex')) packages.push('texlive-xetex');
  if (names.includes('pdfunite')) packages.push(manager === 'apt-get' ? 'poppler-utils' : 'poppler-utils');
