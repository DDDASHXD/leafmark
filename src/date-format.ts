...
function monthNames(parts: DateParts, locale: string): { long: string; short: string } {
 ...
 */
export function formatDateParts(parts: DateParts, pattern: string, locale = 'en'): string {
...
export function formatDocumentDate(rawDate: string, formatPattern: string, locale = 'en'): string {
  const parts = parseDateInput(rawDate);
  if (!parts) {
    throw new Error(
      `Could not parse date "${rawDate}". Use ISO form (2026-02-16) or day/month/year (16/02/2026).`
    );
  }
