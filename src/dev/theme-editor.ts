...
const server = createServer((request, response) => {
  ...
  if (request.method === 'POST' && request.url === '/render') {
    queue = queue.then(() => render(request, response)).catch((error) => json(response, 500, { error: String(error) }));
    return;
  }
...
function cleanup(): void { server.close(); rmSync(scratch, { recursive: true, force: true }); }
process.once('SIGINT', cleanup); process.once('SIGTERM', cleanup);

async function render(request: IncomingMessage, response: ServerResponse): Promise<void> {
  try {
    const payload = JSON.parse(await body(request)) as { theme?: BasicTheme };
    if (!payload.theme || typeof payload.theme !== 'object' || Array.isArray(payload.theme)) throw new Error('Expected a theme object.');
    writeFileSync(join(project, '.leafmark', 'config.json'), JSON.stringify({ order: ['sample.md'], frontmatter: false, metadata: { title: 'Theme specimen', subtitle: 'A live Leafmark PDF', 'title-page': false, toc: false }, theme: payload.theme }, null, 2));
    const tsx = join(packageRoot, 'node_modules', 'tsx', 'dist', 'cli.mjs');
    const cli = join(packageRoot, 'src', 'build.ts');
    const result = await spawnComplete(process.execPath, [tsx, cli, project, 'sample.md', '--output', output, '--skip-tools-check'], { cwd: packageRoot });
    if (result.status !== 0) return json(response, 422, { error: cleanError(result.stderr || result.stdout) });
...
function body(request: IncomingMessage): Promise<string> {
  return new Promise((resolveBody, reject) => { let value = ''; request.setEncoding('utf-8'); request.on('data', chunk => { value += chunk; if (value.length > 200_000) reject(new Error('Request is too large.')); }); request.on('end', () => resolveBody(value)); request.on('error', reject); });
}
function html(response: ServerResponse): void { response.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' }); response.end(UI); }
function json(response: ServerResponse, status: number, value: unknown): void { if (response.headersSent) return; response.writeHead(status, { 'content-type': 'application/json' }); response.end(JSON.stringify(value)); }
function cleanError(value: string): string { return value.replace(/\x1b\[[0-9;]*m/g, '').trim().slice(-4000); }

function sampleMarkdown(): string { return `# A practical theme specimen
